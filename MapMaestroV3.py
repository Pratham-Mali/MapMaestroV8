...
class ElegantMapMaestroApp:
    ...
    def setup_styles(self):
        ...
        # Configure combobox dropdown list font (makes dropdown items bigger)
    ...
    def create_ui(self):
        ...
        self.style.configure("TNotebook.Tab", font=("Segoe UI", 13, "bold"))
    ...
    def create_action_section(self, parent):
        ...
        # Bind action change to show/hide custom rules
        ...
        # ===== CUSTOM RULES SECTION (for Generate Mapping tab) =====
        self.main_custom_rules_container = tk.Frame(action_container, bg=self.colors["white"])
        self.main_custom_rules_container.grid(
            row=1, column=0, columnspan=2, sticky="ew", pady=(0, 15)
        )
        self.main_custom_rules_container.grid_columnconfigure(1, weight=1)
        
        # "Use Custom Rules" checkbox
        self.main_use_custom_rules = tk.BooleanVar(value=False)
        self.main_custom_rules_check = tk.Checkbutton(
            self.main_custom_rules_container,
            text="Use Custom Rules",
            variable=self.main_use_custom_rules,
            command=self.on_main_custom_rules_toggle,
        ...
        self.main_custom_rules_check.config(font=("Segoe UI", 18, "bold"))
        self.main_custom_rules_check.grid(row=0, column=0, columnspan=2, sticky="w", pady=(0, 15))
        
        # Custom rules form frame (initially hidden)
        self.main_custom_rules_form = ttk.Frame(self.main_custom_rules_container, style="Card.TFrame", padding=15)
        self.main_custom_rules_form.grid(row=1, column=0, columnspan=2, sticky="ew")
        self.main_custom_rules_form.grid_columnconfigure(0, weight=1)
        
        # Custom Rules Text Box
        custom_rules_label = ttk.Label(
            self.main_custom_rules_form,
            text="Custom Rules",
            style="FilterLabel.TLabel",
            font=("Segoe UI", 15, "bold"),
        )
        custom_rules_label.grid(row=0, column=0, sticky="w", pady=(0, 8))
        
        custom_rules_text_frame = ttk.Frame(self.main_custom_rules_form)
        custom_rules_text_frame.grid(row=1, column=0, sticky="ew", pady=(0, 15))
        ...
        self.main_custom_rules_text = tk.Text(
        ...
        self.main_custom_rules_text.grid(row=0, column=0, sticky="ew")
        custom_rules_scrollbar.config(command=self.main_custom_rules_text.yview)
        
        # Placeholder text
        placeholder = "Example:\nsalary\ncompensation\nbonus_amount"
        self.main_custom_rules_text.insert("1.0", placeholder)
        self.main_custom_rules_text.config(foreground=self.colors["muted"])
        
        # Bind focus events for placeholder
        self.main_custom_rules_text.bind("<FocusIn>", self.on_main_custom_rules_focus_in)
        self.main_custom_rules_text.bind("<FocusOut>", self.on_main_custom_rules_focus_out)
        
        # Options frame (Scope and Action side by side)
        options_frame = tk.Frame(self.main_custom_rules_form, bg=self.colors["white"])
        ...
        self.main_custom_scope = ttk.Combobox(
            scope_frame,
        ...
        self.main_custom_scope.current(0)  # Default: Field Name
        self.main_custom_scope.pack(fill="x")
        ...
        self.main_custom_action = tk.StringVar(value="PII")
        ...
        pii_radio = ttk.Radiobutton(
            action_radio_frame,
            text="PII",
            variable=self.main_custom_action,
            value="PII",
            command=self.on_main_custom_action_change,
        )
        ...
        self.main_pii_type_frame = tk.Frame(self.main_custom_rules_form, bg=self.colors["white"])
        self.main_pii_type_frame.grid(row=3, column=0, sticky="ew", pady=(0, 0))
        
        pii_type_label = ttk.Label(
            self.main_pii_type_frame,
            text="PII Category",
            style="FilterLabel.TLabel",
            font=("Segoe UI", 15, "bold"),
        )
        ...
        self.main_custom_type = ttk.Entry(
            self.main_pii_type_frame,
            style="Modern.TEntry",
            font=self.fonts["body"],
        )
        self.main_custom_type.insert(0, "Custom_PII")
        self.main_custom_type.grid(row=1, column=0, sticky="ew")
        
        # Initially hide the entire custom rules container and the form
        self.main_custom_rules_container.grid_remove()
        self.main_custom_rules_form.grid_remove()
    ...
    def on_action_change(self, event=None):
        """Handle action dropdown change to show/hide custom rules section"""
        action = self.action_cb.get()
        
        if action == "PII Field Detection":
            # Show custom rules container
            self.main_custom_rules_container.grid()
        else:
            # Hide custom rules container and reset
            self.main_custom_rules_container.grid_remove()
            self.main_use_custom_rules.set(False)
            self.main_custom_rules_form.grid_remove()
    
    def on_main_custom_rules_toggle(self):
        """Handle 'Use Custom Rules' checkbox toggle in Generate Mapping tab"""
        if self.main_use_custom_rules.get():
            # Show the custom rules form
            self.main_custom_rules_form.grid()
        else:
            # Hide the custom rules form
            self.main_custom_rules_form.grid_remove()
    
    def on_main_custom_rules_focus_in(self, event):
        """Remove placeholder text on focus (Generate Mapping tab)"""
        current_text = self.main_custom_rules_text.get("1.0", "end-1c")
        if current_text.startswith("Example:"):
            self.main_custom_rules_text.delete("1.0", tk.END)
            self.main_custom_rules_text.config(foreground=self.colors["dark"])
    
    def on_main_custom_rules_focus_out(self, event):
        """Add placeholder text if empty (Generate Mapping tab)"""
        current_text = self.main_custom_rules_text.get("1.0", "end-1c").strip()
        if not current_text:
            placeholder = "Example:\nsalary\ncompensation\nbonus_amount"
            self.main_custom_rules_text.insert("1.0", placeholder)
            self.main_custom_rules_text.config(foreground=self.colors["muted"])
    
    def on_main_custom_action_change(self):
        """Handle Mark As radio button change (Generate Mapping tab)"""
        # PII Type field visibility is handled by layout
        pass
    ...
    def show_table_dialog(self, title, message, table_data, headers):
        """
        Show a custom dialog with compact grid-style table
        
        Args:
            title: Dialog title
            message: Message text before table
            table_data: List of lists for table rows
            headers: List of column headers
        """
        dialog = tk.Toplevel(self.root)
        dialog.title(title)
        dialog.geometry("550x450")  # Smaller, more compact
        dialog.resizable(False, False)  # Fixed size to prevent layout issues
        ...
        # Table container for better control
        table_container = tk.Frame(scrollable_frame, bg=self.colors["white"])
        table_container.pack(fill="x")
        
        # Table frame with border
        border_color = "#666666"
        
        table_outer = tk.Frame(table_container, bg=border_color, highlightthickness=1, highlightbackground=border_color)
        table_outer.pack()
        
        # Inner table frame
        table_inner = tk.Frame(table_outer, bg=self.colors["white"])
        table_inner.pack()
        
        # Configure grid weights for equal column widths
        table_inner.grid_columnconfigure(0, weight=3, minsize=200)  # Metric column - wider
        table_inner.grid_columnconfigure(1, weight=1, minsize=80)   # Value column - narrower
        ...
        for row_idx, row in enumerate(table_data):
            ...
            if all(cell == "" for cell in row):
                ...
                for col_idx in range(len(headers)):
                    ...
                    sep_label.grid(row=row_num, column=col_idx, sticky="nsew")
                row_num += 1
                continue
            ...
            for col_idx, cell in enumerate(row):
                ...
                cell_label.grid(row=row_num, column=col_idx, sticky="nsew")
    ...
    def _extract_custom_rules_generic(self, text_widget, scope_widget, action_var, 
                                       type_widget, enabled_var=None, tab_name=""):
        """
        Generic method to extract custom rules from any set of widgets.
        This is the SINGLE SOURCE OF TRUTH for custom rules extraction logic.
        
        Args:
            text_widget: tk.Text widget containing patterns
            scope_widget: ttk.Combobox for scope selection
            action_var: tk.StringVar for PII/Non-PII selection
            type_widget: ttk.Entry for PII category
            enabled_var: Optional tk.BooleanVar for checkbox (Generate Mapping only)
            tab_name: Optional tab identifier for logging
        
        Returns:
            List of dict with custom rules, or empty list if not enabled/no rules
        """
        # Check if enabled (for tabs with checkbox)
        if enabled_var is not None:
            if not enabled_var.get():
                return []  # Not enabled, return empty
        
        # Get text from text widget
        custom_text = text_widget.get("1.0", "end-1c").strip()
        ...
        scope_display = scope_widget.get()  # "Field Name", "Field Description", "Table Name"
        action = action_var.get()  # "PII" or "Non-PII"
        pii_type = type_widget.get().strip() if action == "PII" else "Custom_Non_PII"
        
        # Map display names to internal names (SHARED LOGIC)
        scope_map = {
        ...
        # Parse patterns (one per line) (SHARED LOGIC)
        patterns = [line.strip() for line in custom_text.split('\n') if line.strip()]
        
        if not patterns:
            return []
        
        # Create rule object (SHARED LOGIC)
        custom_rule = {
            'patterns': patterns,
        ...
        # Logging with tab context
        log_prefix = f"Custom rule extracted{' from ' + tab_name if tab_name else ''}:"
    ...
    def _extract_custom_rules(self):
        """
        Extract custom rules from PII Field tab.
        This is a simple wrapper around the generic extraction method.
        
        Returns:
            List of dict with custom rules, or empty list if no rules provided
        """
        return self._extract_custom_rules_generic(
            text_widget=self.pii_custom_rules_text,
            scope_widget=self.pii_custom_scope,
            action_var=self.pii_custom_action,
            type_widget=self.pii_custom_type,
            enabled_var=None,  # No checkbox in PII Field tab (always enabled)
            tab_name="PII Field tab"
        )

    def _extract_main_custom_rules(self):
        """
        Extract custom rules from Generate Mapping tab.
        This is a simple wrapper around the generic extraction method.
        ...
        """
        return self._extract_custom_rules_generic(
            text_widget=self.main_custom_rules_text,
            scope_widget=self.main_custom_scope,
            action_var=self.main_custom_action,
            type_widget=self.main_custom_type,
            enabled_var=self.main_use_custom_rules,  # Checkbox in Generate Mapping tab
            tab_name="Generate Mapping tab"
        )
            ...
            try:
                ...
                if action == "Generate mapping spec excels for all subdomains":
                    ...
                    try:
                        ...
                        # Extract custom rules from Generate Mapping tab (if enabled)
                        custom_rules_list = self._extract_main_custom_rules()
                        
                        if custom_rules_list:
                            logger.info(f"Custom rules detected from Generate Mapping tab: {len(custom_rules_list)} rule(s)")
                        else:
                            logger.info("No custom rules provided in Generate Mapping tab, using system rules only")
                        
                        # Create orchestrator with custom rules from Generate Mapping tab
                        conn_pii = get_connection()
                        orchestrator = PIIDetectionOrchestrator(
                            gemini_api_key=gemini_api_key,
                            db_connection=conn_pii,
                            batch_size=12,
                            custom_rules=custom_rules_list  # Use custom rules from Generate Mapping tab
                        )
                        ...
                        # Prepare table data for Combined Summary
                        if total_field_count > 0:
                            detection_rate = (total_pii_count / total_field_count * 100)
                            table_data = [
                                ["Total Files Processed", len(generated_files)],
                                ["Successful", success_count],
                                ["Failed", failed_count],
                                ["", ""],  # Empty row
                                ["Total PII Fields Found", total_pii_count],
                                ["Total Fields Analyzed", total_field_count],
                                ["Overall Detection Rate", f"{detection_rate:.1f}%"]
                            ]
                        else:
                            table_data = [
                                ["Total Files Processed", len(generated_files)],
                        ...
                        # Build message with file details
                        message = details + "\n\nCombined Summary:\n"

                        # Add custom rules info if used
                        custom_rules_info = ""
                        if custom_rules_list:
                            custom_rules_info = f" (with {len(custom_rules_list)} custom rule(s))"

                        # Update status
                        if failed_count == 0:
                            status_msg = (
                                f"PII Detection Complete{custom_rules_info}! {success_count}/{len(generated_files)} files processed. "
                            ...
                            status_msg = (
                                f"PII Detection Completed{custom_rules_info} with errors. "
